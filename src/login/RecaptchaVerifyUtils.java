...
public class RecaptchaVerifyUtils {
    ...
    public static void verify(String gRecaptchaResponse) throws Exception {
        ...
        if (jsonObject.has("error-codes") && jsonObject.get("error-codes").isJsonArray()) {
            throw new Exception("reCAPTCHA verification failed: " + jsonObject.getAsJsonArray("error-codes").toString()+"-->Refresh and Retry please");
        } else {
            throw new Exception("reCAPTCHA verification failed: Unknown error"+"-->Refresh and Retry please");
        }
    }
